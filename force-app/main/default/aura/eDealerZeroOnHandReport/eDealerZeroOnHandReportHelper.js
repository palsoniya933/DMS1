({
    ...
    getReportData : function(component, event, helper, isDownload, offset) {    
        ...
        component.set("v.listDealerCode", lstSelectedDealerCodes);
        ...
        if(filterSelected.length > 0){
            ...
            component.set("v.filter", filterSelected[0].fieldName);
            component.set("v.filterValue", filterSelected[0].fieldValue);
            component.set("v.operator", filterSelected[0].fieldOperator);
        }            
        ...
        console.log('dealerDivision:::'+JSON.stringify(component.get("v.dealerDivision")));
        ...
        var action = component.get("c.fetchAWSZeroOnHandReportData");
        action.setParams({
        ...
        action.setCallback(this, function(response) {            
            ...
            if (state === "SUCCESS") { 
                ...
                if(!$A.util.isEmpty(resultData.error)){
                    ...
                    if(!isDownload){
                        component.set("v.zeroOnHandReportData", resultData.response);
                        console.log('zeroOnHandReportData>>>>>>>>>',JSON.stringify(resultData.response));
                        ...
                        component.set("v.isDisabledReportButton", true);
    ...
    setGridColumns : function(component, event, helper) {
        ...
        console.log('data===>>'+languageTranslation);
        ...
        for (const key in languageTranslation) {
            ...
            if(value == 'oh' || value == 'consecutive_days_stocked_out' || value=='days_since_ship' || 
               value=='mtd_dmd' || value=='dif_on_order' || value=='distrb_on_order' || value=='dmd_12m' ||
               value=='hits_12m' || value=='minss'|| value=='std_pkg_qty'|| value=='est_lead_time_days' || value=='dlrnet'){
    ...
    getLanguageTranslation : function(component, event, helper) {
        var action = component.get('c.getTranslateColumnHeaders'); 
        action.setCallback(this, function(response){
            var state = response.getState(); // get the response state
            if(state == 'SUCCESS') {
                
                component.set("v.languageTranslation", response.getReturnValue());
                console.log('languageTranslation--->',JSON.stringify(component.get("v.languageTranslation")));
                helper.setGridColumns(component, event, helper);
            }
        });
        $A.enqueueAction(action);
