({
    ...
    buildQuery : function(datasetId, currentVersionId){
        ...
        query += "\";q = foreach q generate q.'COUNTRY' as 'COUNTRY', q.'DEALERGROUPNUMBER' as 'DEALERGROUPNUMBER', q.'ABBREVIATEDNAME' as 'ABBREVIATEDNAME', q.'DEALERCODE' as 'DEALERCODE', q.'LOYALTY_REGION' as 'LOYALTY_REGION', q.'DIVISION' as 'DIVISION',q.'DIVISION_formula' as 'DIVISION_formula',q.'LoyaltyRegionCountry' as 'LoyaltyRegionCountry';q = limit q 3000;"
        return query;
    },
    ...
    setFilters : function(component, helper,lstSelectedLocation){
        ...
        let filter = {
            "datasets":{
                "Offer_Pdfs_Dataset": helper.getOfferPdfsFilter(component, selectedDealersInfo)
            }
        };
    ...
    getOfferPdfsFilter : function(component, selectedDealersInfo){
        const brands = [];
        const regions = [];
        let dealerBrandName  = '';
        ...
        if(selectedDealersInfo && selectedDealersInfo.length > 0){
            ...
            for(const dealerInfo of selectedDealersInfo){
                ...
                if(!regions.includes(dealerInfo.LOYALTY_REGION)){
                    ...
                    dealerRegionName = dealerRegionName ? dealerRegionName :dealerInfo.LoyaltyRegionCountry; 
                }
            }
        }
        else{
    ...
    publishData : function(component){
        const brand = component.get('v.brand');
        const region = component.get('v.region');
        const language = component.get('v.language');
        const message = brand + '-' + region + '-' + language;
        var payload = {
            message: message
        };
        component.find("loyaltyMessageChannel").publish(payload);
    },
