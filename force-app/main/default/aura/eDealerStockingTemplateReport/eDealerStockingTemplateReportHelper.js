({
    ...
    getStockingTemplateReportData : function(component, event, helper, isDownload, offset) {    
        debugger;
        ...
        var action = component.get("c.fetchStockingTemplateReport");
        action.setParams({
        ...
        action.setCallback(this, function(response) {            
            ...
            if (state === "SUCCESS") { 
                ...
                if(!$A.util.isEmpty(resultData.error)){
                    ...
                    if(!isDownload){
                        component.set("v.StockingTemplateReportData", resultData.response);
    ...
    setGridColumns : function(component, event, helper) {
        ...
        for (const key in languageTranslation) {
            ...
            if(value == 'abc_level' || value == 'starlevel' || value =='cost' || value == 'mpscovdur' || value == 'ssrule' || value == 'minss' || value == 'sscov'){
                
                columns.push({label: value, fieldName: key.toLowerCase(), type: 'Number', sortable: false});
            }
            else if(!$A.util.isEmpty(currencyCode) && key == 'cost' ){
