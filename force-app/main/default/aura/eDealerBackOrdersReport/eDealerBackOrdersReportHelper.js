({
    ...
    getReportData : function(component, event, helper, isDownload, offset) { 
        try{
            ...
            var action = component.get("c.fetchAWSBackOrdersReportData");
            action.setParams({
            ...
            action.setCallback(this, function(response) {  
                ...
                if (state === "SUCCESS") { 
                    ...
                    if(!$A.util.isEmpty(resultData.error)){
                        ...
                        if(!isDownload){
                            component.set("v.backOrdersReportData", resultData.response);
    ...
    handleReportSelection : function(component, event, helper) {
        ...
        if(!$A.util.isUndefinedOrNull(tableData) && tableData.length > 0){
            ...
            for(var i=0;i<tableData.length;i++){
                if(!$A.util.isUndefinedOrNull(tableData[i].ord_type) && !set1.has(tableData[i].ord_type)){
                    set1.add(tableData[i].ord_type);
                    var objTemp = {};
                    objTemp.label = tableData[i].ord_type;
                    objTemp.checked = false;
                    objTemp.name = tableData[i].ord_type;
        ...
        for (const key in languageTranslation) {
            ...
            if(value == 'qty_order' || value == 'ext_net' || value=='oh'){
                columns.push({label: value, fieldName: key.toLowerCase(), type: 'Number', sortable: false});
            }
            else if(!$A.util.isEmpty(currencyCode) && key == 'ext_net' ){
    ...
    updateBooks: function (cmp, fieldName) {
        var rows = cmp.get('v.backOrdersReportData');
