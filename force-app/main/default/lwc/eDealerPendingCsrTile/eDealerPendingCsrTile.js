...
import getPendingCSRTileTranslations from '@salesforce/apex/EDealerPendingCsrController.getPendingCSRTileTranslations';
import getCSRs from '@salesforce/apex/EDealerPendingCsrController.getPendingCSR';
...
export default class EDealerPendingCsrTile extends LightningElement {
    @api dealercode;
    @api dealercodelist;
    @api selectedlocobj
    ...
    @track pendingCSROptions = [
    ];
    selectedPendingCSRType;
    translations;
    title;
    hasDealerLoc;
    csrData;
    ...
    inProcessRecords
    waitingRecords
    Edealer_Pending_CSR_Tile_Help 
    Edealer_Pending_CSR_Tile_ErrorMsg
    connectedCallback() {
        console.log('selectedlocobj'+this.selectedlocobj);
        console.log('dealercodelist'+this.dealercodelist);
        this.displayLoading = true;
        this.csrData = undefined;
        this.selectedPendingCSRType = 'All';
        this.pendingCsrCount = 0;
        this.getPendingCSRTileTranslations();
        this.getCSRs();
    }

    getPendingCSRTileTranslations(){
        getPendingCSRTileTranslations()
        .then((result) => {
            this.error = undefined;
            this.translations = result;
            this.title = result.csrTitle.label;
            this.pendingCSROptions = result.csrType;
            this.Edealer_Pending_CSR_Tile_Help = result.helpText.label
            this.Edealer_Pending_CSR_Tile_ErrorMsg = result.errorMsg.label
          
        }).catch((error) => {
            this.displayLoading = false;
            this.title = undefined;
            this.translations = undefined;
            this.pendingCSROptions = undefined;
    ...
    getCSRs(){
        ...
        .then((result) => {
            ...
            console.log(result)
            if(this.csrData){
                const scsrcount = this.csrData[this.selectedTimespan]['S'] ? this.csrData[this.selectedTimespan]['S'] : 0 ;
                const dcsrcount = this.csrData[this.selectedTimespan]['D'] ? this.csrData[this.selectedTimespan]['D'] : 0 ;
                const mcsrcount = this.csrData[this.selectedTimespan]['M'] ? this.csrData[this.selectedTimespan]['M'] : 0 ;
                const ucsrcount = this.csrData[this.selectedTimespan]['U'] ? this.csrData[this.selectedTimespan]['U'] : 0 ;
                const acsrcount = this.csrData[this.selectedTimespan]['A'] ? this.csrData[this.selectedTimespan]['A'] : 0 ;
                const xcsrcount = this.csrData[this.selectedTimespan]['X'] ? this.csrData[this.selectedTimespan]['X'] : 0 ;
                const icsrcount = this.csrData[this.selectedTimespan]['I'] ? this.csrData[this.selectedTimespan]['I'] : 0 ;
                const wcsrcount = this.csrData[this.selectedTimespan]['W'] ? this.csrData[this.selectedTimespan]['W'] : 0 ;
              
                
                this.inProcessRecords = scsrcount+dcsrcount+mcsrcount+ucsrcount+acsrcount+xcsrcount+icsrcount ; 
                this.waitingRecords = wcsrcount ; 
    ...
    handleSubTypeUpdate(event) {
        this.selectedPendingCSRType = event.detail.value;
        this.updateUI();
    }
    ...
    updateUI(){
        const dealercodes = typeof this.dealercodelist == 'string'? JSON.parse(this.dealercodelist): this.dealercodelist ;
        if(this.selectedPendingCSRType =='Process'){
            this.pendingCsrCount = this.inProcessRecords;  
        }
       else if(this.selectedPendingCSRType =='Waiting'){
            this.pendingCsrCount = this.waitingRecords;  
        }
        else{
            this.pendingCsrCount = this.inProcessRecords + this.waitingRecords;
        }
        this.hasDealerLoc = ((dealercodes!= null && dealercodes.length==1) && this.pendingCsrCount != 0 ) ? true : false;
        this.enableSamePageListing = (dealercodes!= null && this.pendingCsrCount != 0 ) ? true : false;
    ...
    openCsrListing() {
        ...
        const message = { 
            "detail" : 'showlisting',
            "listingType" : 'pendingcsrlisting',
            "dealercodes" : selectedobj,
            "frequency": this.selectedTimespan,
            "csrtype":  this.selectedPendingCSRType
